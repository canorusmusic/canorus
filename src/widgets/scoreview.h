...
public:
    ...
    inline bool removeFromSelection(CADrawableMusElement* elt)
    {
        return _selection.removeAll(elt);
        emit selectionChanged();
    }
    ...
    inline void setShadowNoteVisible(bool visible)
    {
        _shadowNoteVisible = visible;
        setShadowNoteVisibleOnLeave(visible);
    }
...
private:
    ...
    template <typename T>
    double getMaxXExtended(CAKDTree<T>& v); // Make the viewable World a little bigger (stuffed) to make inserting at the end easier
    template <typename T>
    double getMaxYExtended(CAKDTree<T>& v); // Make the viewable World a little bigger (stuffed) to make inserting below easies
